...
class DiffAlign(nn.Module):
    ...
    def __init__(
        ...
        # Flow Matching (no schedules needed)
        ...
        # ---- Flow Matching ----
        # No noise schedules needed - just use continuous time t ∈ [0,1]
        self.num_timesteps = int(num_timesteps)  # keep for reference
    ...
    def FlowMatching_Sampling_UFF(
        ...
        *,
        ...
        uff_start_ratio: float = 0.0,         # skip UFF if t < ratio
        # ---- UFF dynamic nonbonded params ----
        uff_vdw_multiplier: float = 10.0,     # dynamic cutoff multiplier
        debug_log: bool = False,
    ):
        """
        Flow Matching sampler with UFF guidance.
        Simple Euler integration: x_{t+dt} = x_t + v_t * dt
        """

        device = query_batch.pos.device if hasattr(query_batch, 'pos') else 'cpu'
        ...
        for step in range(num_steps):
            t = step * dt   # t goes from 0 to 1
            t_tensor = torch.full((qb.num_graphs,), t, device=device, dtype=torch.float32)

            # Current coords + predict velocity
            cur_q = qb.clone()
            cur_q.pos = x_t
            
            # CFG (classifier-free guidance)
            if cfg_scale == 1.0:
                v_hat_merged = self(cur_q, rb, t_tensor, condition=True)
            else:
                v_u = self(cur_q, rb, t_tensor, condition=False)
                v_c = self(cur_q, rb, t_tensor, condition=True)
                v_hat_merged = v_u + cfg_scale * (v_c - v_u)
            
            # Extract query velocity
            merged = merge_graphs_in_batch(cur_q, rb, device=device)
            qmask = (merged.graph_idx % 2 == 0)
            v_hat = v_hat_merged[qmask]

            # UFF Guidance (apply to current position x_t)
            if use_uff and t >= uff_start_ratio and uff_guidance_scale > 0:
                
                # Extract ligand coords [B, Nq, 3]
                coords_q0 = x_t.index_select(0, gather_idx_q).view(B, Nq, 3).detach()

                # Build full coords (ligand+pocket) for neighbor refresh
                coords_full = torch.cat([coords_q0, pocket_coords_fixed], dim=1)

                with torch.no_grad():
                    uff_model._refresh_nonbond_candidates(coords_full)

                # Gradient descent on x_t
                coords_q = coords_q0.clone()
                inner = max(1, int(uff_inner_steps))
                step_scale = uff_guidance_scale / float(inner)
                ...
                # Apply refined coords back to x_t
                x_t_guided = x_t.clone()
                x_t_guided.index_copy_(0, gather_idx_q, coords_q.reshape(B * Nq, 3))
                x_t = x_t_guided
